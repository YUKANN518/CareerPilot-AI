...
test("portfolio main flow: confirmed resume to evidence-grounded match report", async ({
  ...
  await expect(page.getByTestId("skills-MISSING")).toContainText("No verified evidence found")
  ...
  await expect(page.getByRole("heading", { name: "Blocking Risk" })).toBeVisible()
})
